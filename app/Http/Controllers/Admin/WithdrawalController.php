...
class WithdrawalController extends Controller
{
    public function pending()
    {
        ...
        $withdrawals = Withdraw::where('status', 0)->with(['user'])->latest()->paginate(getPaginate());
        $withdrawal = Withdraw::with(['user'])->latest()->paginate(getPaginate());

        $empty_message = 'No withdrawal is pending';
        $type = 'pending';
        return view('admin.withdraw.withdrawals', compact('page_title', 'withdrawals', 'withdrawal', 'empty_message', 'type'));
    }
    public function approved()
    {
        $page_title = 'Approved Withdrawals';
        $withdrawals = Withdraw::where('status', 1)->with(['user'])->latest()->paginate(getPaginate());
        $withdrawal = Withdraw::with(['user'])->latest()->paginate(getPaginate());
    ...
    public function rejected()
    {
        ...
        $withdrawals = Withdraw::where('status', 3)->with(['user'])->latest()->paginate(getPaginate());
        $withdrawal = Withdraw::with(['user'])->latest()->paginate(getPaginate());
    ...
    public function log()
    {
        ...
        $withdrawals = Withdraw::with(['user'])->latest()->paginate(getPaginate());
        $withdrawal = Withdraw::with(['user'])->latest()->paginate(getPaginate());
        $empty_message = 'No withdrawal history';
        return view('admin.withdraw.withdrawals', compact('page_title', 'withdrawals', 'withdrawal', 'empty_message'));
    }
    ...
    public function details($id)
    {
        ...
        return view('admin.withdraw.detail', compact('page_title', 'withdrawal', 'details'));
    }
