...
class DepositController extends Controller
{
    ...
    public function pending()
    {
        ...
        $type = 'pending';
        $deposits = Deposit::where('status', 0)->with(['user'])->latest()->paginate(getPaginate());
        $deposit = Deposit::with(['user'])->latest()->paginate(getPaginate());
        return view('admin.deposit.log', compact('page_title', 'empty_message', 'deposits', 'type', 'deposit'));
    }
    ...
    public function deposit()
    {
        ...
        $deposits = Deposit::with(['user'])->latest()->paginate(getPaginate());
        $deposit = Deposit::with(['user'])->latest()->paginate(getPaginate());
        return view('admin.deposit.log', compact('page_title', 'empty_message', 'deposits', 'deposit'));
    }
    ...
    public function details($id)
    {
        ...
        $page_title = $deposit->user->username . ' requested ' . getAmount($deposit->amount);
    ...
    public function approve(Request $request)
    {
        ...
        $user->total_deposit = getAmount($user->total_deposit + $deposit->amount);
        $user->status = 1;
        $user->save();
        ...
        $transaction->details = 'Deposit approved for ' . $user->username . ' with an amount of ' . $request->amount;
