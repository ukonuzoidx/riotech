<x-authentication-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="space-y-4">
            ...
            <div>
                <x-jet-label for="username">{{ __('Username') }} <span class="text-rose-500">*</span></x-jet-label>
                <x-jet-input id="username" type="text" name="username" :value="old('username')" required />
            </div>

            <div>
                <x-jet-label for="ref_code">{{ __('Referral Code') }}</x-jet-label>
                <x-jet-input id="ref_code" type="text" name="ref_code" :value="old('ref_code')" />
            </div>

            <div>
                <x-jet-label for="busd_wallet">{{ __('BUSD Wallet') }} <span class="text-rose-500">*</span></x-jet-label>
                <x-jet-input id="busd_wallet" type="text" name="busd_wallet" :value="old('busd_wallet')" required />
            </div>
